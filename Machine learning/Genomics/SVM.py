...
def create_feature_importance_plot(trained_model, feature_import, title, outpath, save_fp):
    ...
    feature_import.to_csv(outpath.parent / "SVM_features.csv")
...
def main(outpath):
    ...
    f_importances(abs(clf.coef_[0]), f, top=10)
    ...
    model_svc = LinearSVC()
    estimator = CalibratedClassifierCV(model_svc)
    ...
    feat_plot_title = 'Three Class SVM Reciever Operating Characteristic Curve'
    ...
    ftr_plot_title = 'Support Vector Machine feature importance'
    create_feature_importance_plot(model, feature_import, ftr_plot_title, outpath / "SVM_feature_plot.png", save_fp)
    ...
    fig.savefig(outpath / "SVM_roc_plot.png")


if __name__ == '__main__':
    output_basepath = pathlib.Path("results")
    outpath = output_basepath / "SVM"
    ...
    sys.stdout = open(outpath / "SVM_log.txt", "w")
